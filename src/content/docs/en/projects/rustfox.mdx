---
title: Hyprland
description: Hyprland environment in Axyl Projects.
---
...
In Axyl Projects, Hyprland has been been set up to make the user experience smoother in a window manager environment.
...
Axyl Projects's Hyprland config is based on the wonderful [dotfiles from @end-4](https://github.com/end-4/dots-hyprland).

![Hyprland](@assets/hyprland-example.png)
...
![Hyprland Top Bar](@assets/hyprland-topbar.png)
...
![Sidebars](@assets/hyprland-sidebars.png)
...
![Hyprland Desktop](@assets/hyprland-sweetdark.png)
...
![Hyprland Activity Overview](@assets/hyprland-activity-overview.png)

![Hyprland App Search](@assets/hyprland-search.png)
...
To install Axyl Projects Hyprland environment on side of an existing environment, run:
```shell
sudo pacman -S Axyl Projects-hyprland-config
dconf load /org/gnome/desktop/ < /usr/share/Axyl Projects-hyprland-config/dconf-desktop.ini
```
Then reboot. On Login Manager, select **Hyprland** session and login.

If your theme is not correctly displayed, run `Axyl Projects-theme-tweak` to set or change the current environment theme.
