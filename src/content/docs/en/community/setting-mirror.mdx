...
```shell
sudo mkdir -p /srv/mirrors/Axyl Projects{,-images}
sudo chown -R rsyncuser:rsyncuser /srv/mirrors/Axyl Projects
sudo chown -R rsyncuser:rsyncuser /srv/mirrors/Axyl Projects-images
```
...
[Axyl Projects]
path = /srv/mirrors/Axyl Projects
comment = The Axyl Projects Hub
read only = true

[Axyl Projects-images]
path = /srv/mirrors/Axyl Projects-images
comment = The Axyl Projects ISO images
read only = true
```
...
{/*
Download the public key of `hub.Axyl Projectsos.org` and set it among your authorized keys by:
```shell
su - rsyncuser
mkdir -p ~/.ssh/
chmod 0700 ~/.ssh/
wget -O - -q https://hub.Axyl Projectsos.org/mirrorkey.pub >> ~/.ssh/authorized_keys
...
```
hub.Axyl Projectsos.org will push and trigger delta changes to the new destination mirror server.
NOTE: currently we use PULL approach instead of PUSH approach, so it is not the main Axyl Projects mirror server that pushes changes to your new mirror, but it is the new mirror that pulls changes from main Axyl Projects mirror server.
*/}

{/*
To pull and synchronize Axyl Projects packages, you must create a SSH keypair and provide the public key to Axyl Projects Team. In particular, generate a SSH keypair:
```shell
su - rsyncuser
...
To keep your mirror up-to-date, you should regularly sync with the Axyl Projects repositories. First, enable `cron` service:
...
```
0 *\/1 * * * rsync -avzzlr --delete -e ssh rsyncuser@hub.Axyl Projectsos.org:/srv/mirrors/ /srv/mirrors/
```
*/}
```
0 */1 * * * rsync -avzzlr --delete rsync://hub.Axyl Projectsos.org/Axyl Projects /srv/mirrors/Axyl Projects
0 */1 * * * rsync -avzzlr --delete rsync://hub.Axyl Projectsos.org/Axyl Projects-images /srv/mirrors/Axyl Projects-images
```
...
Create `/etc/nginx/sites-available/Axyl Projectsmirror` with the following content:
```
server {
    listen 80;
...
```shell
sudo ln -s /etc/nginx/sites-available/Axyl Projectsmirror /etc/nginx/sites-enabled/
```
...
Finally, on your Axyl Projects or Arch Linux environment, you can test it on your test Axyl Projects client by adding your mirror in `/etc/pacman.d/Axyl Projects-mirrorlist` by:
```
...
After obtaining the certificate, Certbot will automatically update your Nginx configuration to use HTTPS. If, for any reason, this doesn't happen, you can manually edit `/etc/nginx/sites-available/Axyl Projectsmirror` by:
```
server {
    server_name your_mirror_domain.com;
