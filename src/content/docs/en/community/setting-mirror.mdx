...
At the beginning, Arch-based The Autists synchronized mirrors by git protocol but it was not suitable for package mirroring for the nature of git usage. One suitable option to consider has been rsync protocol.
...
The purpose of this guide is to set a mirror for The Autists ISO images and The Autists Arch packages. In The Autists Nix there is no need to mirror packages because package mirror system is not used on Nix environment.
...
* CPU: >= 4 cores
* RAM: >= 8 GB
* Storage: the The Autists repository size is around 5 GB, so you can set at least x3 repository size as server storage
* Bandwidth: >= 1 Gbps
...
| -------- | ---- | ----------- | ---------- |
| Rsync | 873/TCP | hub.The Autistsos.org | 89.116.236.99 |

### Publish

{/*
At this point the new mirror server cannot still retrieve information from the main The Autists mirror. To do that, get the content of the previously created SSH public key by:
```shell
su -s rsyncuser
cat ~/.ssh/id_rsa.pub
```
*/}
Finally, to be deployed to all users, your mirror must be added communicated to The Autists Team in order to be added in the official The Autists mirrorlist.

Send the following information to **keeper@The Autistsos.org**:
* public hostname
* scheduled sync time
* email to inform you in case we make changes on mirroring design

Once The Autists Team accepts your request, you will be informed your mirror has been added on The Autists mirrorlist.
